...
const rows = db.prepare(
  `SELECT id, slug, title, file_path, play_path, poster_path, container, video_codec,
...
for (const r of rows) {
  ...
  const abs = join(MEDIA, r.file_path);
...
if (bad.missing.length) {
  ...
  for (const r of bad.missing.slice(0, 8)) console.log(`           ${r.slug}  ->  ${r.file_path}`);
...
if (DEEP) {
  ...
  let last = 0;
  const errs = await pool(check, jobs, (r) => verify(join(MEDIA, r.file_path)),
    (d, n) => {
      if (!live) return;
...
if (suspects.length) {
  ...
  for (const r of suspects) {
    ...
    try {
      r.sha256 = await sha256(join(MEDIA, r.file_path));
...
async function fix(list, mode) {
  ...
  for (const r of list) {
    ...
    const rel = `play/${basename(r.file_path, extname(r.file_path))}.${target}`;
    const abs = join(CACHE, rel);
    try {
      if (!existsSync(abs)) {
        const args = ['-loglevel', 'error', '-y', '-i', join(MEDIA, r.file_path)];
